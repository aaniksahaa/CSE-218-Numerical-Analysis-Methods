...
def powermodel(xs,ys):
    ...
    for i in range(n):
        newxs[i] = np.log(xs[i])  # transform x
        newys[i] = np.log(ys[i])  # transform y
...
def powermodel_predict(xs,ys,allx):
    allx = np.array(allx)
    a,b = powermodel(xs,ys)
    n = allx.shape[0]
    ally = np.zeros(n)
    for i in range(n):
        ally[i] = (a*(allx[i]**b)) # write the given equation in terms of allx array
    return ally

# returns Sum of squares of residuals
def powermodel_SSE(xs,ys):
    xs = np.array(xs)
    n = xs.shape[0]
    predicted_y = powermodel_predict(xs,ys,xs)
    SSE = 0.0
    for i in range(n):
        SSE += (ys[i]-predicted_y[i])**2
    return SSE

def powermodel_graph(xs,ys,predictx):  
    ...
    ally = powermodel_predict(xs,ys,allx)
    plt.title("Plot for Regression")
    ...
    predicty = powermodel_predict(xs,ys,predictx)
...
xs = [1,1.5,2,3,3.5,4]
ys = [4.54,1.144,0.43,0.11,0.064,0.041]
...
powermodel_graph(xs,ys,predictx)
