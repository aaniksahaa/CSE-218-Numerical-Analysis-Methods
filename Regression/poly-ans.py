...
def expreg_direct(xs,ys):
    ...
    def func(b):
        ...
        if(R==0):
            print("Please Handle division by Zero")
        return P - (Q*S)/R
...
def printPolynomial(xs,ys,m):
    print("Upon Calculation and Observation, Polynomial Regression with power {} best fits the data\n\n".format(m))
    coeffs = polyreg(xs,ys,m)
    coeffs = np.array(coeffs)
    print("Best Fit Model  =  " + "%.10f"%coeffs[0] + " ",end="")
    for i in range(1,coeffs.shape[0]):
        print("+ ("+"%.10f"%coeffs[i] + ")(x^{})".format(i) + " ", end="")
    print()
    print()
...
"""
xs = np.arange(1900,2001,10)
ys = [10.3,13.5,13.9,14.2,11.6,10.3,9.7,9.6,14.1,19.8,31.1]
...
#ys = [1000,550,316,180,85,56,31]
compareModels(xs,ys)
printPolynomial(xs,ys,3)
print()
givenx = 2010
predictedy = polyreg_predict(xs,ys,[float(givenx)],3)

print("Predicted Value for x = {}, y = ".format(givenx) + "%.10f"%predictedy[0])
print()
print()

polyreg_graph(xs,ys,3)
