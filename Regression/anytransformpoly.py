...
def anytransform(xs,ys):
    ...
    for i in range(n):
        newxs[i] = xs[i]  # transform x
        newys[i] = xs[i]*ys[i]  # transform y

    coeffs = polyreg(newxs,newys,2)  # Use tranformed regression

    c = coeffs[0]
    a = coeffs[1]       # convert to new parameters
    b = coeffs[2]
...
def anytransform_predict(xs,ys,allx):
    allx = np.array(allx)
    a,b,c = anytransform(xs,ys)
    n = allx.shape[0]
    ally = np.zeros(n)
    for i in range(n):
        ally[i] = a + b*allx[i] + c/allx[i]  # write the given equation in terms of allx array
    return ally
