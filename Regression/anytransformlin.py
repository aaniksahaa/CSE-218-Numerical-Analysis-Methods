...
def anytransform(xs,ys):
    ...
    for i in range(n):
        newxs[i] = 1/(xs[i]*xs[i])  # transform x
        newys[i] = 1/ys[i]  # transform y
...
def anytransform_predict(xs,ys,allx):
    ...
    ally = np.zeros(n)
    for i in range(n):
        ally[i] = (a*allx[i]*allx[i])/(b+allx[i]*allx[i])  # write the given equation in terms of allx array
    return ally
