...
def expreg_direct(xs,ys):
    ...
    # guess for b
    loguess = 10 # subject to change
    higuess = 30 # subject to change
    err = 0.001
    mx = 10000
...
def printExponential(xs,ys):
    print("Upon Calculation and Observation, Exponential Regression best fits the data\n\n")
    a,b = expreg_direct(xs,ys)
    print("Best Fit Model  =  " + "%.10f"%a + "*e^(" + "%.10f"%b + "x)",end="")
    print()
    print()
...
xs = [0,0.01, 0.03, 0.05, 0.07, 0.09, 0.11, 0.13, 0.15, 0.17, 0.19, 0.21]
ys = [1,1.03, 1.06, 1.38, 2.09, 3.54, 6.41, 12.6, 22.1, 39.05, 65.32, 99.78]
...
printExponential(xs,ys)

predict = expreg_direct_predict(xs,ys,[0.16])
print(predict[0])

expreg_direct_graph(xs,ys)
